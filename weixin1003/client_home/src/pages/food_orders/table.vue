<template>
  <view class="page_food_orders diy_table diy_table_tml" id="food_orders_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                      <uni-forms-item label="食物名称" name="food_name">
                      <uni-easyinput type="text" v-model="query.food_name" placeholder="食物名称" />
                    </uni-forms-item>
                        <uni-forms-item label="制作状态" name="production_status">
                 <uni-data-select placeholder="请选择"
                        v-model="query['production_status']"
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/food_orders/view?" v-show="user_group == '管理员' || $check_action('/food_orders/table','add') || $check_action('/food_orders/view','add')">添加</navigator>
                            ...
                            <span>顾客用户</span>
								...
								<text>
									{{ get_user_customer_users(o['customer_users']) }}
													...
													<view v-if="1 && $check_field('get','food_name')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>食物名称</span>
                          </view>
								...
								<text>
									{{ o["food_name"] }}
													...
													<view v-if="1 && $check_field('get','purchase_quantity')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>购买数量</span>
                          </view>
                            ...
                            <span>订单总价</span>
								...
								<text>
									{{ o["total_order_price"] }}
													...
													<view v-if="1 && $check_field('get','production_status')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>制作状态</span>
                          </view>
								...
								<text>
									{{ o["production_status"] }}
													...
													<view v-if="0 && $check_field('get','order_remarks')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>订单备注</span>
                          </view>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/food_orders/table','del') || $check_action('/food_orders/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/food_orders/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/food_orders/table','set') || $check_action('/food_orders/view','set') || $check_action('/food_orders/view','get')" class="btn btn-primary">详情</navigator>
	        					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/food_orders/get_list?like=0",
				url_del: "~/api/food_orders/del?",
                ...
                list_user_customer_users: [],
                                                // 制作状态列表
                list_production_status: [{value:"",text:"全部"}],
                			}
		},
		methods: {
          search() {
